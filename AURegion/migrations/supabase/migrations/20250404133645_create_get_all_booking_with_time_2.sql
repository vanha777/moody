...
BEGIN
    ...
    WITH latest_bookings AS (
        SELECT 
            ...
            ROW_NUMBER() OVER (PARTITION BY b.customer_id ORDER BY b.start_at DESC) as rn
        FROM public.booking b
        WHERE b.company_id = p_company_id
        AND (p_start_date IS NULL OR b.start_at >= p_start_date)
        AND (p_end_date IS NULL OR b.start_at <= p_end_date) -- Changed to include today
        AND NOT EXISTS (
            SELECT 1 
        ...
        GROUP BY b.customer_id, DATE(b.start_at)
    )
    SELECT 
        lb.customer_id,
        lb.booking_date,
